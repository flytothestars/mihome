...
<h1 class="page-title">
    ...
    <a href="{{ route('voyager.'.$dataType->slug.'.restore', $dataTypeContent->getKey()) }}"
        title="{{ __('voyager::generic.restore') }}" class="btn btn-all btn-default restore"
        data-id="{{ $dataTypeContent->getKey() }}" id="restore-{{ $dataTypeContent->getKey() }}">
        <i class="voyager-trash"></i> <span class="hidden-xs hidden-sm">{{ __('voyager::generic.restore') }}</span>
    </a>
    @else
    <a href="javascript:;" title="{{ __('voyager::generic.delete') }}" class="btn btn-all btn-danger delete"
    ...
    <a href="{{ route('voyager.'.$dataType->slug.'.index') }}" class="btn btn-all btn-warning">
        ...
        <div class="col-md-12">
            ...
            <div class="panel panel-bordered" style="padding-bottom:5px;">
                ...
                <div class="panel-body" style="padding-top:0;">
                    ...
                    @endif
                    <h4>Заказ: {{ $dataTypeContent->id }}</h4>
                    <p><strong>Офферы:</strong></p>
                    <ul>
                        @foreach($dataTypeContent->offers as $offer)
                        <li>
                            Оффер: {{ $offer->id }}
                            <ul>
                                <li>Продукт: {{ $offer->product->name }}</li>
                            </ul>
                        </li>
                        @endforeach
                    </ul>
        ...
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <form action="{{ route('voyager.'.$dataType->slug.'.index') }}" id="delete_form" method="POST">
                    {{ method_field('DELETE') }}
                    {{ csrf_field() }}
                    <input type="submit" class="btn btn-all btn-danger pull-right delete-confirm"
                        value="{{ __('voyager::generic.delete_confirm') }} {{ strtolower($dataType->getTranslatedAttribute('display_name_singular')) }}">
                </form>
                <button type="button" class="btn btn-all btn-default pull-right" data-dismiss="modal">{{
